...
優先查：對最終結論影響大、confidence低、來源互相衝突、recommendation對它敏感的主張。編制1辯護＋3反方（claim挑刺／矛盾搜索／來源多樣性），同模型三prompt並行。contest（互斥結論、單源高風險推論）才加輪；論點不再推進就停，硬上限3輪；停火後標 GENUINELY_UNCERTAIN 並寫下何種證據可解。異議（多數 verdict＋直接引文）寫進「核心爭議」區。
